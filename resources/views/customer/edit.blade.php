...
@section('content')
    ...
    {{-- {{dd($customer)}} --}}
    <form action="{{ route('customer.update', $customer->uuid) }}" id="customer-form" method="POST">
        @csrf
        <div class="card bg-white shadow-xl p-5">
            ...
            <div class="form-control">
                ...
                <select class="select select-bordered w-full bg-slate-100 @error('province') mt-2 text-sm text-red-500 @enderror" name="province" id="province-select">
                    <option selected>{{explode('-', $customer->province)[1]}}</option>
                </select>
                <div class="invalid-feedback">
                    @error('business_phone')
                        {{ $message }}
            ...
            <div class="form-control city-input">
                ...
                <select class="select select-bordered w-full bg-slate-100" name="city" id="city-select">
                    <option selected>{{ explode('-', $customer->city)[1] }}</option>
                </select>
                <div class="invalid-feedback">
                    @error('business_phone')
                        {{ $message }}
            ...
            <div class="form-control district-input">
                ...
                <select class="select select-bordered w-full bg-slate-100" name="district" id="district-select">
                    <option selected>{{explode('-', $customer->district)[1]}}</option>
                </select>
                <div class="invalid-feedback">
                    @error('business_phone')
                        {{ $message }}
            ...
            <div class="form-control urban-village-input">
                ...
                <select class="select select-bordered w-full bg-slate-100" name="urban_vilage" id="urban-village-select">
                    <option selected>{{explode('-', $customer->urban_village)[1]}}</option>
                </select>
                <div class="invalid-feedback">
                    @error('business_phone')
                        {{ $message }}
            ...
            <div class="form-control">
                ...
                <textarea class="textarea textarea-bordered bg-slate-100" placeholder="Alamat" name="address">{{$customer->address}}</textarea>
                <div class="invalid-feedback">
                    @error('business_phone')
                        {{ $message }}
                    @enderror
            ...
            <div class="form-control">
                ...
                <textarea class="textarea textarea-bordered bg-slate-100" placeholder="Kode pos atau lainnya" name="additional_address">{{$customer->additional_address}}</textarea>
                <div class="invalid-feedback">
                    @error('business_phone')
                        {{ $message }}
                    @enderror
        ...
        <div class="mt-4 flex justify-end">
            <button type="submit" class="btn bg-primary border-primary hover:bg-primary-dark hover:border-primary-dark shadow-lg text-white" id="store-customer">
